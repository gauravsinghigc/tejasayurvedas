...
if (isset($_POST['UpdatePrimaryConfigurations'])) {
 ...
 //update installation & shipping
} elseif (isset($_POST['INSTALLATION_SHIPPING'])) {
 $INSTALLATION_SHIPPING = SECURE($_POST['INSTALLATION_SHIPPING'], "e");

 $data = array(
  "configurationvalue" => SECURE($_POST['INSTALLATION_SHIPPING'], "e"),
  "configurationname" => "INSTALLATION_SHIPPING",
 );

 $check = CHECK("SELECT * FROM configurations where configurationname='INSTALLATION_SHIPPING'");
 if ($check == null) {
  $Update = INSERT("configurations", $data);
 } else {
  $Update = UPDATE("UPDATE configurations SET configurationvalue='$INSTALLATION_SHIPPING' where configurationname='INSTALLATION_SHIPPING'");
 }

 RESPONSE($Update, "Shipping & Installation Description are updated Successfully!", "Unable to update shipping & installation description!");
}
