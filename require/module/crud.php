...
//universal System In-OUT Request handler
function SIO($tablename = null, $ReqtablePrimaryId = null, array $Req = null, array $msg = null, $die = false)
{
    //check update request for not
    if (isset($_POST['Update'])) {
        $AccessId = SECURE($_REQUEST["$ReqtablePrimaryId"], "d");
        $Update = SECURE($_POST["Update"], "d");
    } else {
        $AccessId = null;
        $Update = false;
    }

    //run save/update query
    if ($Update == true) {
        $Query = UPDATE_DATA("$tablename", $Req, "$ReqtablePrimaryId='$AccessId'", $die);
    } else {
        $Query = INSERT("$tablename", $Req, $die);
    }

    //send response
    RESPONSE($Query, $msg['true'], $msg['false']);
}


//controller/Request Handler for POST, GET, REQUEST methods
function IfIsset($Request = null, array $req = null, $msg = null, $die = false)
{

    if (isset($Request['req'])) {
        SIO($Request['db'], $Request['id'], $req, $msg, $die);
    } else {
        $msg['null'] = "Invalid controller access Request is received!";
